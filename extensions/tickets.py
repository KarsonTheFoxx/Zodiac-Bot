...
# View that is on the ticket embed
class closeTicketView(ui.View):
    def __init__(self):
        super().__init__(timeout=0)
    # Button setup, callback | Creates button, deletes channel
...
# View that is on the message sent to embed_channel
class createTicketView(ui.View):
    ...
    # Button setup, callback | Creates button, opens a modal. responds to button interaction
...
# Modal for ticket creation form
class createTicketModal(ui.Modal):
    ...
    # Modal callback | Creates ticket, replies to modal interaction
    async def callback(self, inter:ModalInteraction):
        ...
        while ticket_name in ticket_names: # Ensures channel name is unique even if the author has more than 10 channels
...
# Modal for the embed creation form
class newTicketEmbedModal(ui.Modal):
    ...
    # Modal callback | sends a message, responds to modal interaction
    async def callback(self, inter:ModalInteraction):
        embed = Embed()
...
# Root call of the interactions. Calls are effectively:
# create_ticket_embed_command | The slash command | embed_channel: The channel to send the prompt to
# -> newTicketEmbedModal | The setup screen for the ticket prompt
# --> createTicketView | The button attached to the ticket prompt
# ---> createTicketModal | The setup for a new ticket
# ----> closeTicketView | The button attached to the tickets embed
...
async def create_ticket_embed_command(inter:CommandInteraction, embed_channel:TextChannel, category_channel:CategoryChannel, moderator_role:Role):
